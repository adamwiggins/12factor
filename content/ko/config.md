...
이 "설정"의 정의는 애플리케이션 내부 설정을 *포함하지 않는다는 점*에 유의해야 합니다. Rails의 `config/routes.rb`이나 [Spring](http://www.springsource.org/)의 ["어떻게 코드 모듈이 연결되는 가](http://static.springsource.org/spring/docs/2.5.x/reference/beans.html)과 같은 설정들은 배치 사이에서 변하지 않기 때문에 코드의 내부에 있는 것이 가장 좋습니다.
